 ...
 */
void
process_data(int sock, char *buf, struct sockaddr_in from, socklen_t fromLen,
    ...
    packet_h curheader;
    if ((short) header->seqNo <= session->last_packet_acked ||
        (short) header->seqNo >
    ...
    if ((session->last_packet_acked + 1) == (short) header->seqNo) {
        //debug purpse
        char *packet_hash = get_chunk_hash(buf,
        ...
        if ((uint32_t) session->last_packet_acked < header->seqNo) {
            ...
            session->recved_flags[header->seqNo - session->last_packet_acked -
                                  1] = 1;
        }
    }
    /* send ACK packet */
        ...
        for (int i = 0; i < data->len; i++) {
            ...
            if (!strcmp(d->chunk_hash, session->chunk_hash)) {
                ...
                if (!strcmp(chunk_hash, session->chunk_hash)) {
                    ...
                    fprintf(stdout,
                            "The hash of received data matches with chunk hash\n");
                } else {
                    //todo: need to build a recovery mechanism in corrupted data
                    fprintf(stdout,
                            "Corrupted chunk data receied due to unmatched hash\n");
                }
                free(chunk_hash);
                break;
            }
        ...
        if (check_data_complete(recv_sessions, &config->request_queue,
            ...
            for (int i = 0; i < queued_requests->len; i++) {
                ...
                if (!strcmp(r->ip, ip) && port == r->port) {
                    ...
                    token = strtok(NULL, " ");
                    udp_recv_session *new_session = (udp_recv_session *) malloc(
                            sizeof(udp_recv_session));
                    peer_info_t *peer = find_peer_from_list(config->peer, ip,
                                                            port);
                    build_udp_recv_session(new_session, peer->id, token,
                                           config);
